    ...
    xs
      .merge(
        xs
          .merge(
            validResponse$,
            lostOrFoundPerson$.filter(
              input => input.type == InputType.FOUND_PERSON
            )
          )
        ...
        xs
          .merge(
            speechSynthesisActionResult$,
            lostOrFoundPerson$.filter(
              input => input.type == InputType.LOST_PERSON
            )
          )
  ...
  const transitionTable = {
    ...
    [State.LISTEN]: {
      [InputType.VALID_RESPONSE]: (prevVariables, prevInputValue) => ({
        state: State.SAY,
        variables: {
          sentence: flowchart[prevVariables.sentence][prevInputValue]
        },
        outputs: {
          SpeechSynthesisAction: {
...
function main(sources) {
  ...
  const machine$ = input$.fold(
    (machine, input) => transition(machine.state, machine.variables, input),
    defaultMachine
  );
