...
function main(sources) {
  ...
  var expression$ = sources.TwoSpeechbubblesAction.result.debug().map(function (result) {
    if (result.result === 'Good') {
      return 'happy';
    } else if (result.result === 'Bad') {
      return 'sad';
...
function SpeechbubbleAction(sources) {
        ...
        if (goal === null) {
            ...
            return {
                ...
                value: !value.goal.type ? {
                    goal_id: value.goal_id,
                    goal: {
                        type: typeof value.goal === 'string' ? SpeechbubbleType.MESSAGE : SpeechbubbleType.CHOICE,
                        value: value.goal
                    }
                } : value
            };
    ...
    var click$ = sources.DOM.select('.choice').elements().map(function (b) {
            ...
            if (state.status === action_1.Status.ACTIVE) {
                switch (state.goal.type) {
                    ...
                    case SpeechbubbleType.CHOICE:
                        return snabbdom_pragma_1.default.createElement("span", null, state.goal.value.map(function (text) {
                            return snabbdom_pragma_1.default.createElement("button", { className: "choice" }, text);
                        }));
                }
            } else {
                return null;
...
function main(sources) {
    ...
    var action$ = xstream_1.default.merge(goal$, first.result.map(function (result) {
        return { type: 'FIRST_RESULT', value: result };
    }), second.result.map(function (result) {
        return { type: 'SECOND_RESULT', value: result };
    }));
    var initialState = {
        goal: null,
        goal_id: action_1.generateGoalID(),
    ...
    var state$ = action$.fold(function (state, action) {
        ...
        if (state.status === action_1.Status.SUCCEEDED || state.status === action_1.Status.PREEMPTED || state.status === action_1.Status.ABORTED) {
            ...
            if (action.type === 'GOAL') {
                ...
                if (state.goal.type === TwoSpeechbubblesType.ASK_QUESTION && action_1.isEqual(state.goal_id, action.value.status.goal_id)) {
    ...
    var goals$ = stateStatusChanged$.filter(function (state) {
        return state.status === action_1.Status.ACTIVE || state.status === action_1.Status.PREEMPTED;
    }).map(function (state) {
        if (!state.goal) {
            return {
    ...
    var firstGoal$ = goals$.map(function (state) {
        return state.first;
    });
    var secondGoal$ = goals$.map(function (state) {
        return state.second;
    });
    return {
        DOM: adapt_1.adapt(xstream_1.default.combine(first.DOM, second.DOM).map(function (_a) {
            var fdom = _a[0],
                sdom = _a[1];
            return snabbdom_pragma_1.default.createElement("div", null, snabbdom_pragma_1.default.createElement("div", null, snabbdom_pragma_1.default.createElement("span", null, "Robot:"), " ", snabbdom_pragma_1.default.createElement("span", null, fdom)), snabbdom_pragma_1.default.createElement("div", null, snabbdom_pragma_1.default.createElement("span", null, "Human:"), " ", snabbdom_pragma_1.default.createElement("span", null, sdom)));
        })),
        status: adapt_1.adapt(status$),
        result: adapt_1.adapt(result$),
        targets: {
...
function powerup(main, connect) {
    return function (sources) {
        ...
        var targets = sinks.targets,
            sinksNoTargets = __rest(sinks, ["targets"]);
        return sinksNoTargets;
    };
}
function TwoSpeechbubblesAction(sources) {
    return powerup(main, function (proxy, target) {
        return proxy.imitate(target);
    })(sources);
}
...
var EyeController = /** @class */function () {
    ...
    EyeController.prototype._createKeyframes = function (_a) {
        var tgtTranYVal = _a.tgtTranYVal,
            tgtRotVal = _a.tgtRotVal,
            enteredOffset = _a.enteredOffset,
            exitingOffset = _a.exitingOffset;
...
function TabletFace(sources, _a) {
    ...
    var load$ = faceElement$.filter(function () {
        return true;
