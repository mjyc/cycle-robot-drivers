...
function makeConcurrentAction(actionNames, isRace) {
    ...
    var input = function (goal$, cancel$, results) {
        ...
        return xstream_1.default.merge(goal$.filter(function (g) {
            ...
            return g === null ? { type: SIGType.CANCEL, value: null } : { type: SIGType.GOAL, value: utils_1.initGoal(g) };
        }), cancel$.map(function (val) {
            return { type: SIGType.CANCEL, value: val };
        }), results$.map(function (r) {
            return { type: SIGType.RESULTS, value: r };
        }));
    };
        ...
        var transitionReducer$ = input$.map(function (input) {
            return function (prev) {
                ...
                if (prev.state === S.PEND && input.type === SIGType.GOAL) {
                    ...
                    if (!isRace && results.every(function (r) {
                    ...
                    } else if (!!isRace && results.some(function (r) {
                        return utils_1.isEqualGoalID(r.status.goal_id, prev.variables.goal_id) && r.status.status === types_1.Status.SUCCEEDED;
                    })) {
                        ...
                        var finishedActionNames_1 = results.map(function (r, i) {
                            return utils_1.isEqualGoalID(r.status.goal_id, prev.variables.goal_id) ? actionNames[i] : null;
                        });
                        return __assign({}, prev, { variables: __assign({}, prev.variables, { activeActionNames: prev.variables.activeActionNames.filter(function (n) {
                                    return finishedActionNames_1.indexOf(n) === -1;
                                }) }), outputs: null });
                    }
                }
                return prev;
    ...
    var output = function (reducerState$) {
        ...
        return actionNames.reduce(function (acc, x) {
            acc[x] = {
                goal: outputs$.debug().filter(function (o) {
                    return !!o[x];
                }).map(function (o) {
                    return o[x].goal;
                }),
    ...
    return function ConcurrentAction(sources) {
        ...
        var createDummyResult = function () {
            return {
                status: {
...
function generateGoalID(_a) {
    var _b = _a === void 0 ? {} : _a,
        _c = _b.stamp,
        stamp = _c === void 0 ? undefined : _c,
        _d = _b.id,
        id = _d === void 0 ? undefined : _d;
    var now = new Date();
    return {
        stamp: typeof stamp === 'undefined' ? now : stamp,
        id: typeof id === 'undefined' ? Math.random().toString(36).substring(2) + "-" + now.getTime() : id
    };
}
...
function isEqualGoalStatus(first, second) {
    return isEqualGoalID(first.goal_id, second.goal_id) && first.status === second.status;
}
