...
function main(sources) {
  ...
  const speechbubbles$ = xs.merge(
    xs
      .of({ goal_id: { stamp: Date.now(), id: `sb` }, goal: "Hello there!" })
      .compose(delay(1000)),
    xs
      .of({ goal_id: { stamp: Date.now(), id: `sb` }, goal: ["Hello!", "Bye"] })
      .compose(delay(2000)),
  ...
  speechbubbleAction.status.addListener({
    next: s => console.log("SpeechbubbleAction status", s)
  });
  ...
  facialExpressionAction.status.addListener({
    next: s => console.log("FacialExpressionAction status", s)
  });
