...
function main(sources) {
  sources.state.stream.addListener({ next: function next(s) {
      return console.log('reducer state', s);
    } });
...
function SpeechSynthesisAction(sources) {
    var input$ = input(xstream_1.default.fromObservable(sources.goal), xstream_1.default.fromObservable(sources.cancel), xstream_1.default.fromObservable(sources.SpeechSynthesis.events('start')), xstream_1.default.fromObservable(sources.SpeechSynthesis.events('end')));
    var reducer = transitionReducer(input$);
    var result$ = sources.state.stream.map(function (rs) { return rs.result; }).filter(function (rs) { return !!rs; });
    var status$ = status(sources.state.stream);
    var outputs = output(sources.state.stream);
