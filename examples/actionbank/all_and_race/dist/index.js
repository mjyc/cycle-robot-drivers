...
function QAWithScreenAction(sources) {
    ...
    var goal$ = sources.goal
        .filter(function (g) { return typeof g !== 'undefined'; })
        .map(function (g) { return g === null ? null : action_1.initGoal({
        goal_id: g.goal_id,
        goal: {
        ...
        var transitionReducer$ = input$.map(function (input) { return function (prev) {
            ...
            if (prev.state === S.PEND && input.type === SIGType.GOAL) {
                ...
                if (!isRace
                    ...
                    var out = actionNames.reduce(function (acc, x, i) {
                        acc[x] = results_1[i];
                        return acc;
                    }, {});
                    ...
                    var idx = results_1.findIndex(function (r) {
                        return action_1.isEqual(r.status.goal_id, prev.variables.goal_id)
                            && r.status.status === action_1.Status.SUCCEEDED;
                    }); // break the tie here
                    var result = results_1[idx];
                    ...
                    return {
                        ...
                        outputs: {
                            result: {
                                ...
                                result: (_a = {}, _a[actionName] = result.result, _a),
                            },
                        },
                    };
                }
                else {
                    var finishedActionNames_1 = results_1.map(function (r, i) {
                        return action_1.isEqual(r.status.goal_id, prev.variables.goal_id)
                            ? actionNames[i]
                            : null;
                    });
            ...
            else if (prev.state === S.PREEMPTING && input.type === SIGType.RESULTS) {
                var results_2 = input.value
                    .filter(function (r) { return action_1.isEqual(r.status.goal_id, prev.variables.goal_id); });
                if (results_2.length === actionNames.length) {
                    var out = actionNames.reduce(function (acc, x, i) {
                        acc[x] = results_2[i];
                        return acc;
                    }, {});
